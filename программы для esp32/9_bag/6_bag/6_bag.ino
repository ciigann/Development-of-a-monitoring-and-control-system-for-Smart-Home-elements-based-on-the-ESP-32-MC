...
int pointCount = 3; // Изменено значение по умолчанию на 3
...
int prevPointCount = 3; // Изменено значение по умолчанию на 3
...
void saveZoneNewToSD(int zoneNumber) {
  ...
  setupMode = false;
  displayNewZone = true;
  pointCount = 3; // Изменено значение по умолчанию на 3
  zoneCreationMode = false;
  selectedZone = -1;
  deleteMode = false;
  isNewZoneBold = false;
...
void saveZoneToSD() {
  ...
  // Создаем новую зону и добавляем в нее текущие красные точки
  ...
  for (size_t i = 0; i < redPointsX.size(); i++) {
    ...
    int x_mm = map(redPointsX[i], 0, tft.width(), -6000, 6000);
    int y_mm = map(redPointsY[i], 0, tft.height(), 6000, 0);
    newZone.push_back(std::make_pair(x_mm, y_mm));
  }

  // Добавляем новую зону в вектор зон
  ...
  for (size_t i = 0; i < newZone.size(); i++) {
    if (i > 0) {
      file.print(";");
    }
    file.print(newZone[i].first);
    file.print(",");
    file.print(newZone[i].second);
  }
...
void drawZones(int selectedZone = -1) {
  for (size_t i = 0; i < zones.size(); i++) {
    ...
    for (size_t j = 0; j < zones[i].size(); j++) {
      // Преобразуем координаты с противоположным знаком X для отображения
      int x = map(-zones[i][j].first, -6000, 6000, tft.width(), 0);  // Отрицательный X для отображения
      int y = map(zones[i][j].second, 0, 6000, tft.height(), 0);
      tft.fillCircle(x, y, 5, color);
      
      if (j > 0) {
        int prevX = map(-zones[i][j - 1].first, -6000, 6000, tft.width(), 0);  // Отрицательный X для отображения
    ...
    if (zones[i].size() > 1) {
      int firstX = map(-zones[i][0].first, -6000, 6000, tft.width(), 0);  // Отрицательный X для отображения
      int firstY = map(zones[i][0].second, 0, 6000, tft.height(), 0);
      int lastX = map(-zones[i][zones[i].size() - 1].first, -6000, 6000, tft.width(), 0);  // Отрицательный X для отображения
      int lastY = map(zones[i][zones[i].size() - 1].second, 0, 6000, tft.height(), 0);
...
void loop() {
  ...
  if (pointCount == 1 && countdown == 1 && !zoneSaved && newZoneBold) {
    ...
    // Очищаем динамические массивы для хранения координат красных точек
    redPointsX.clear();
    redPointsY.clear();
  ...
  // Отрисовка зон на переднем плане
