...
void saveZoneNewToSD(int zoneNumber) {
  ...
  // Создаем новый файл zone_new.txt и записываем номер зоны
  File file = SD.open("/src/zone_new.txt", FILE_WRITE);
  if (!file) {
    Serial.println("Failed to open file for writing");
    return;
  }

  file.print(zoneNumber);
  file.close();
  Serial.println("Zone number saved to zone_new.txt");
}
