...
// Мьютекс для синхронизации доступа к SD-карте
...
// Переменная для хранения номера зоны, которую нужно удалить
int lineToRemove = -1;
        ...
        if (setupMode) {
          if (zoneCreationMode) {
            ...
            if (isNewZoneBold) {
              ...
              deleteMode = true;
              lineToRemove = boldZoneNumber; // Запоминаем номер зоны для удаления
              ...
              isZoneBold = false;
              boldZoneNumber = -1;
            } else if (deleteMode && boldZoneNumber == -1) {
              // Если нажата надпись "delete" жирным шрифтом, удаляем зону
              if (lineToRemove != -1) {
                removeLine(SD, "/src/zone.txt", lineToRemove);
                zones.erase(zones.begin() + lineToRemove - 1);
                ZoneCount--;
                lineToRemove = -1;
...
void checkFilesystem() {
    ...
    if (!SD.exists("/src/zone.txt")) {
        File f = SD.open("/src/zone.txt", FILE_WRITE);
        if (f) f.close();
    }

    if (!SD.exists("/src/coordinates.txt")) {
        File f = SD.open("/src/coordinates.txt", FILE_WRITE);
        if (f) f.close();
    }

    if (!SD.exists("/src/Wi-Fi.txt")) {
        File f = SD.open("/src/Wi-Fi.txt", FILE_WRITE);
        if (f) f.close();
    }
}
...
void removeLine(fs::FS &fs, const char *path, int lineToRemove) {
    ...
    // 1. Проверка наличия SD-карты
    if (!sdCardPresent) {
        Serial.println("SD card not present");
        return;
    }

    // 2. Захват мьютекса с таймаутом
    ...
    File sourceFile = fs.open(path, FILE_READ);
    if (!sourceFile) {
    ...
    File newFile = fs.open("/src/zone_new.txt", FILE_WRITE);
    if (!newFile) {
    ...
    while (sourceFile.available()) {
        ...
        if (currentLine != lineToRemove) {
            if (!newFile.println(line)) {
                Serial.println("Write error to new file");
                errorOccurred = true;
                break;
            }
        }
    ...
    // 8. Проверка успешности операции
    if (!errorOccurred) {
        // Удаляем исходный файл
        if (!fs.remove(path)) {
            Serial.println("Failed to remove original file");
            fs.remove("/src/zone_new.txt");
            xSemaphoreGive(sdMutex);
            return;
        }

        // Переименовываем новый файл
        if (!fs.rename("/src/zone_new.txt", path)) {
        ...
        // Удаляем частично созданный новый файл при ошибке
        fs.remove("/src/zone_new.txt");
        Serial.println("Operation aborted due to error");
    }

    xSemaphoreGive(sdMutex);
}

void cleanUpFile(fs::FS &fs, const char *path) {
    Serial.printf("Cleaning up file: %s\n", path);
    ...
    File tempFile = fs.open("/temp.txt", FILE_WRITE);
    if (!tempFile) {
    ...
    if (fs.remove(path)) {
        ...
        Serial.println("File cleanup completed");
    } else {
        Serial.println("Failed to remove original file");
        fs.remove("/temp.txt");
    }
...
void saveCoordinatesToSD(int x, int y) {
  ...
  // Используем буферизацию и записываем данные реже
  static String buffer;
  static unsigned long lastWriteTime = 0;

  buffer += String(x) + "," + String(y) + ",";
  if (wifiConnected) {
  ...
  // Записываем на SD-карту только если буфер достаточно большой или прошло время
  if (buffer.length() > 512 || millis() - lastWriteTime > 5000) {
    if (xSemaphoreTake(sdMutex, pdMS_TO_TICKS(1000)) != pdTRUE) {
      Serial.println("Failed to acquire SD mutex");
      return;
    }
    ...
    if (file) {
      ...
      Serial.println("Failed to open coordinates file");
    }
...
void drawZones(int selectedZone) {
...
void setup() {
  ...
  // Создаем мьютекс для синхронизации доступа к SD-карте
