      ...
      if (millis() - lastPressTime < doublePressDelay) {
        ...
        if (setupMode && !displayNewZone) {
          if (zoneCreationMode) {
            ...
            if (selectedZone != -1 && !deleteMode) {
              // Изменяем надпись на "delete" жирным шрифтом
              deleteMode = true;
              tft.fillRect(tft.width() - 100, tft.height() - 40 - (ZoneCount - selectedZone + 1) * 20, 100, 20, ST77XX_BLACK);
              tft.setCursor(tft.width() - 100, tft.height() - 40 - (ZoneCount - selectedZone + 1) * 20);
              ...
              if (selectedZone > 0 && selectedZone <= zones.size()) {
                ...
                ZoneCount--;
                saveAllZonesToSD(); // Сохраняем все зоны на SD-карту
                deleteMode = false;
                selectedZone = -1;
