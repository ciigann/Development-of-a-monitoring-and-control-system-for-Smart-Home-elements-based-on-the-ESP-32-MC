...
void drawZones() {
  for (size_t i = 0; i < zones.size(); i++) {
    ...
    if (zones[i].size() > 1) {
      ...
      tft.drawLine(lastX, lastY, firstX, firstY, ST77XX_RED);
    }
  }
}
    ...
    for (int i = 0; i < ld2450.getSensorSupportedTargetCount(); i++) {
      ...
      if (result_target.valid) {
        ...
        if (setupMode) {
          ...
          if (prevDisplayNewZone != displayNewZone || prevPointCount != pointCount || prevZoneCreationMode != zoneCreationMode || prevCountdown != countdown || prevShowCheckmark != showCheckmark) {
            tft.fillRect(tft.width() - 100, tft.height() - 40, 100, 20, ST77XX_BLACK);
          }

          if (displayNewZone) {
            tft.print("new zone");
          } else if (zoneCreationMode) {
